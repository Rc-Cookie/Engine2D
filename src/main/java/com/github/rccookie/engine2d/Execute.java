...
public final class Execute {
    ...
    public static void repeating(Runnable task, double delay) {
        Execute.repeating(task, delay, 0);
    }

    public static void repeating(Runnable task, double delay, double initialDelay) {
        Execute.repeating(task, delay, initialDelay, false);
    }

    public static void repeating(Runnable task, double delay, double initialDelay, boolean realTime) {
        Execute.repeating(() -> {
            task.run();
            return true;
    ...
    public static void repeating(BooleanSupplier task, double delay) {
        Execute.repeating(task, delay, 0);
    }

    public static void repeating(BooleanSupplier task, double delay, double initialDelay) {
        Execute.repeating(task, delay, initialDelay, false);
    }

    public static void repeating(BooleanSupplier task, double delay, double initialDelay, boolean realTime) {
        RepeatingExecutionTask executionTask = new RepeatingExecutionTask(task, new BooleanSupplier() {
            double nextTime = (realTime ? Time.realTime() : Time.time()) + initialDelay;
            @Override
    ...
    public static void later(Runnable task, double delay) {
        Execute.later(task, delay, false);
    }

    public static void later(Runnable task, double delay, boolean realTime) {
        Execute.later(() -> {
            task.run();
            return null;
        }, delay, realTime);
    ...
    public static <R> Future<R> later(Supplier<R> task, double delay) {
        return Execute.later(task, delay, false);
    }

    public static <R> Future<R> later(Supplier<R> task, double delay, boolean realTime) {
        double executionTime = (realTime ? Time.realTime() : Time.time()) + delay;
        return realTime ?
